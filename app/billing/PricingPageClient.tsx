...
export default function PricingPageClient({
  ...
  const plans = [
    ...
    },
    {
      ...
      notIncluded: ["Dedicated Server Vault", "Custom Planetary Grid"],
      planKey: "PRO",
  ...
  return (
    <main className="min-h-screen bg-black text-white py-24 px-6 relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-125 h-125 bg-primary/10 blur-[150px] rounded-full" />
      <div className="absolute bottom-0 left-0 w-125 h-125 bg-blue-500/5 blur-[150px] rounded-full" />

      <div className="mx-auto max-w-6xl relative z-10">
        <div ref={headerRef} className="text-center space-y-6 mb-20">
          <Badge
            variant="outline"
            className="text-primary border-primary bg-primary/5 px-6 py-1 uppercase tracking-[0.3em] font-black text-[10px]"
          >
            Subscription Matrix
          </Badge>
          ...
          <p className="text-zinc-500 text-lg max-w-2xl mx-auto font-medium">
            All paid protocols are provisioned for a minimum of 1 year to ensure
            operational stability and neural synchronization.
          </p>
        </div>
            ...
            >
              <Card
                className={`flex flex-col h-full border-white/5 bg-zinc-900/40 backdrop-blur-2xl shadow-2xl relative overflow-hidden rounded-[2.5rem] transition-all duration-500 ${
                  plan.popular
                    ? "border-primary/50 shadow-primary/20 ring-1 ring-primary/20"
                ...
                <CardContent className="p-10 pt-0 flex-1 space-y-8">
                  ...
                  <div className="space-y-4">
                    ...
                    {plan.notIncluded.map((feature) => (
                      <div
                        key={feature}
                        className="flex items-center gap-4 opacity-20 grayscale"
                      >
                        <div className="w-1.5 h-1.5 rounded-full bg-zinc-800" />
                        <span className="text-xs font-medium text-zinc-700 uppercase tracking-widest line-through">
                          {feature}
                        </span>
                      </div>
                    ))}
