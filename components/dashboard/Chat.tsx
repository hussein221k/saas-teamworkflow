...
import { Send, Loader2, User, Hash, Search, MoreVertical, Paperclip, Smile } from "lucide-react";
...
function Chat({ teamId, currentUserId }: ChatProps) {
  ...
  const handleSend = useCallback(async () => {
    ...
    setInputValue("");
    await send(content);
    sendLocalNotification("Neural Transmission confirmed", `Message fragment: "${content.substring(0, 20)}..."`);
    ...
    return messages.map((msg: any) => {
      ...
      return (
        <div key={msg.id} className={cn("flex w-full animate-in fade-in slide-in-from-bottom-2 duration-300", isMe ? "justify-end" : "justify-start")}>
          <div className={cn("flex max-w-[80%] gap-3 group", isMe ? "flex-row-reverse" : "flex-row")}>
            <Avatar className="w-9 h-9 border border-white/5 shadow-2xl shrink-0">
              <AvatarFallback className="bg-zinc-800 text-zinc-400 text-xs font-bold">{msg.user?.name?.[0] || "U"}</AvatarFallback>
            </Avatar>
            <div className="flex flex-col">
              <div className={isMe ? "items-end" : "items-start"}>
                <div className="flex items-center gap-2 mb-1 px-1">
                   <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-tighter">{msg.user?.name}</span>
                   <span className="text-[9px] text-zinc-600 font-medium">{format(new Date(msg.createdAt), 'HH:mm')}</span>
                </div>
                <div className={cn(
  ...
  return (
    <div className="flex flex-col h-full w-full bg-zinc-950 border-r border-white/5 overflow-hidden">
        ...
        <div className="flex items-center gap-3">
            ...
            <div>
                <h2 className="text-sm font-bold text-zinc-100 flex items-center gap-2 uppercase tracking-widest italic">
                    ...
                    <Badge variant="outline" className="text-[9px] h-4 border-emerald-500/50 text-emerald-500 bg-emerald-500/5 px-1.5 uppercase tracking-tighter">Live</Badge>
                </h2>
                <p className="text-[10px] text-zinc-500 font-medium tracking-tight">Synchronized with planetary grid.</p>
            </div>
        </div>
      ...
      <ScrollArea className="flex-1 bg-black">
        <div className="p-6 space-y-6 min-h-full flex flex-col justify-end" ref={scrollRef}>
          {loading ? (
            <div className="space-y-6">
                {[1, 2, 3].map((i) => (
                    <div key={i} className={cn("flex w-full gap-3", i % 2 === 0 ? "flex-row-reverse" : "flex-row")}>
                         <Skeleton className="w-9 h-9 rounded-full bg-white/5 shrink-0" />
                         <div className={cn("flex flex-col gap-2", i % 2 === 0 ? "items-end" : "items-start")}>
                            <Skeleton className="h-2 w-20 bg-white/5" />
                            <Skeleton className={cn("h-10 rounded-2xl bg-white/5", i % 2 === 0 ? "w-64" : "w-48")} />
                         </div>
                    </div>
                ))}
            </div>
            ...
            <div className="flex flex-col items-center justify-center h-full text-zinc-600 space-y-4 py-20 grayscale opacity-20">
                ...
                <p className="text-sm font-light uppercase tracking-widest italic">The channel is silent. First blood?</p>
            </div>
          ) : messageList}
        </div>
      </ScrollArea>
      ...
      <div className="p-4 bg-zinc-900/50 backdrop-blur-xl border-t border-white/10 relative overflow-hidden">
        {(loading || isSending) && <div className="absolute top-0 left-0 h-[2px] bg-primary animate-pulse w-full" />}
        
        <div className="flex items-center gap-2 max-w-5xl mx-auto">
            <div className="flex items-center gap-1">
                <Button variant="ghost" size="icon" className="h-9 w-9 text-zinc-500 hover:text-white hover:bg-white/5 transition-colors">
            ...
            <div className="flex-1 relative">
                <Input
                    ...
                    disabled={isSending}
                />
                <Button variant="ghost" size="icon" className="absolute right-1 top-1/2 -translate-y-1/2 h-8 w-8 text-zinc-500 hover:text-primary hover:bg-transparent">
        ...
        <p className="text-[10px] text-zinc-600 text-center mt-3 font-black uppercase tracking-widest opacity-50 italic">Secure transmission active</p>
      </div>
    </div>
  );
}
