  ...
  const handleCreateTeam = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.team) {
        ...
        setNewTeamName("");
        setActiveSheet(ActiveSheet.team);
  ...
  const handleCreateGroup = useCallback(async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setNewGroupName("");
        setActiveSheet(ActiveSheet.group);
        refetchChannels();
      } else {
  ...
  const onJoinTeam = useCallback(async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setActiveSheet(ActiveSheet.join);
        setJoinCodeInput("");
        router.refresh();
      } else {
        toast.error(res.error);
  ...
  return (
    <>
      ...
      >
        ...
        {/* Sheets for Creation */}
        <Sheet
          open={activeSheet === ActiveSheet.group}
          onOpenChange={(open) => !open && setActiveSheet(undefined)}
        >
          <SheetContent
            side="left"
          ...
          >
            <SheetHeader>
              <SheetTitle className="text-white">NEW CLUSTER</SheetTitle>
            </SheetHeader>
            <div className="py-6">
              <Input
                value={newTeamName}
        ...
        >
          ...
          >
            ...
            <Button onClick={handleCreateGroup} className="w-full font-bold">
              SYNC GROUP
            </Button>
          </SheetContent>
        </Sheet>
