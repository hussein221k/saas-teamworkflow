...
import { Brain, X, Sparkles, MessageSquare, Terminal, Zap, ShieldCheck, Loader2 } from "lucide-react";
  ...
  const [messages, setMessages] = useState<any[]>(() => {
    if (typeof window !== 'undefined') {
        ...
        return saved ? JSON.parse(saved) : [
            { role: 'system', content: 'Cognitive engine online. How can I assist with your workflow data today?' }
        ];
    }
  ...
  const handleSend = useCallback(async (text?: string) => {
    ...
    setTimeout(() => {
        ...
        if (query.toLowerCase().includes("task")) {
            response = "I've analyzed your current task distribution. Efficiency is at 84%. I recommend finalizing the 'Sync Protocols' mandate.";
        } else if (query.toLowerCase().includes("team")) {
            response = "Team Alpha remains stable. Connectivity across all nodes is verified.";
        } else {
            response = "Input processed. Integrating your query into the current operational context. Would you like a detailed neural breakdown?";
        }
  ...
  return (
    <>
      <div className={cn(
        ...
        isDesktop ? "h-screen border-l border-white/5 shadow-[-20px_0_50px_-15px_rgba(0,0,0,0.5)]" : "fixed inset-0 w-full",
        isOpen ? (isDesktop ? "w-[400px]" : "w-full") : "w-0 p-0 overflow-hidden"
      )}>
        {/* AI Header */}
        <div className="p-6 border-b border-white/5 bg-zinc-900/20 backdrop-blur-md flex items-center justify-between">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h2 className="text-sm font-bold tracking-tight text-white uppercase italic">Neural Core</h2>
                    <div className="flex items-center gap-2">
                        <span className={cn("w-1.5 h-1.5 rounded-full animate-pulse", isThinking ? "bg-yellow-500" : "bg-green-500")} />
                        <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">{isThinking ? "Thinking..." : "Active"}</span>
                    </div>
                </div>
            </div>
            <Button variant="ghost" size="icon" onClick={() => setIsOpen(false)} className="text-zinc-500 hover:text-white hover:bg-white/5">
                <X className="w-4 h-4" />
            </Button>
        </div>
        ...
        <div className="px-6 py-4 border-b border-white/5 grid grid-cols-2 gap-3 bg-zinc-900/50">
            ...
            <div className="p-3 rounded-xl border border-white/5 bg-black/40">
                 <div className="flex items-center gap-2 mb-1 opacity-50">
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-wider">Integrity</span>
                </div>
            ...
            <div className="space-y-6" ref={scrollRef}>
                {messages.map((m, i) => (
                    <div key={i} className={cn(
                        ...
                        <div className={cn(
                            ...
                            m.role === 'system' ? 'bg-primary/20 border-primary/20 text-primary shadow-lg shadow-primary/10' : 'bg-zinc-800 border-white/5 text-zinc-400'
                        )}>
                            {m.role === 'system' ? <Terminal className="w-4 h-4" /> : <MessageSquare className="w-4 h-4" />}
                        </div>
                        <div className={cn(
                            "p-4 rounded-2xl text-xs leading-relaxed max-w-[85%] font-medium",
                            m.role === 'system' ? 'bg-zinc-900/80 text-zinc-300 border border-white/5' : 'bg-primary text-primary-foreground'
                        )}>
                            {m.content}
                        </div>
                    </div>
                ...
                {isThinking && (
                    <div className="flex gap-3 animate-pulse">
                        ...
                        <div className="p-4 rounded-2xl bg-zinc-900/80 text-zinc-500 text-xs italic">Processing neural paths...</div>
                    </div>
                )}
            </div>
        </ScrollArea>
